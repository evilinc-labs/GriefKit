...
/**
 * Advanced rotation system with priority queue, silent rotations,
 * movement fix, and mouse sensitivity correction.
 * Ported from GriefKit — adapted for anarchy-client's EventBus.
 */
public class RotationUtils {
    ...
    private boolean movementFix = false; // TODO port InputAccessor mixin to re-enable
    ...
    public float getWrappedYaw() { return MathHelper.wrapDegrees(serverYaw); }
    public float getServerPitch() { return serverPitch; }
    public float getLastServerYaw() { return lastServerYaw; }
    public float getLastServerPitch() { return lastServerPitch; }
    ...
    public static Vec3d getHitVector(Entity entity, HitVector hitVector) {
        ...
        return switch (hitVector) {
            ...
            case CLOSEST -> {
                ...
                if (feetDist <= torsoDist && feetDist <= eyesDist) yield feetPos;
                else if (torsoDist <= eyesDist) yield torsoPos;
                else yield eyesPos;
            }
        };
    }
